...
不需要下载应用安装包，也不需要先打开终端。只要把下面两段话依次复制并发给 WorkBuddy 或 Codex 的智能体。
...
```text
请下载安装 WorkBuddy Theme Manager Skill：
https://github.com/yyyy004/workbuddy-theme-manager
```

等待智能体告诉你“Skill 已安装”或“安装完成”。
...
```text
请安装并启用 WorkBuddy Theme Studio，并告诉我如何打开主题设置。
```

智能体会检查你的 WorkBuddy 是否兼容，然后自动创建 Theme Studio、备份官方应用，并在账号菜单中增加 **主题**。这一步不会自动更换你的主题。
